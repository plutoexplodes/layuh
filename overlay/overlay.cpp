...
void overlay::load_interface()
{
    ...
    ::RegisterClassExW(&wc);
    HWND hwnd = ::CreateWindowExW(WS_EX_TOPMOST, wc.lpszClassName, L"Layuh", WS_POPUP, 0, 0, GetSystemMetrics(SM_CXSCREEN), GetSystemMetrics(SM_CYSCREEN), nullptr, nullptr, wc.hInstance, nullptr);
    ...
    while (done == false)
    {
        ...
        if (FindWindowA(0, "Roblox") && (GetForegroundWindow() == FindWindowA(0, "Roblox") || GetForegroundWindow() == hwnd))
        {
            ...
            if (overlay::visible || ui_alpha > 0.01f)
            {
                ...
                ImGuiWindowFlags layuh_flags = ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize;
                if (!overlay::visible) layuh_flags |= ImGuiWindowFlags_NoInputs;
                if (ImGui::Begin("layuh", nullptr, layuh_flags))
                {
                    // Custom header: centered title "layuh.public" with red ".public"
                    ...
                    dl_root->AddText(ImVec2(tx + s1.x, ty), IM_COL32(255, 64, 64, 255), t2);

                    // Move content below header
                    ImGui::SetCursorPosY(ImGui::GetCursorPosY() + header_h);
                    // Horizontal tab navigation (j1e style)
                    static int selected_tab = 0;
                    const char* tabNames[] = { "camlock", "silent", "misc", "checks", "visuals", "movement", "menu", "config" };
